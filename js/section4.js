var Slider = function(id, _web, _tab, _mobile, spacing){
  ...
  var DOM = {
    ...
    slider: function(container){
      ...
      dom.className = 'exhibition_slide'
      dom.style.position = 'relative';
      dom.style.overflow = 'hidden';
      dom.style.height = '100%';
      dom.style.left = 0;
      dom.style.transition = 'left .9s';
      return dom;
    }
  }
  ...
  var items = document.querySelector('#'+ id + ' .exhibition_slide').children;
  ...
  function resize(){
    left = 0;
    document.querySelector('#'+ id + ' .exhibition_slide').style.left = left + 'px';
  ...
  function setDisplayCount(count) {
    ...
    if(innerWidth <600){
      sliderItemWidth = containerWidth / display;
    } else if(innerWidth >= 600 && innerWidth){
      sliderItemWidth = containerWidth / 2;
    }

    document.querySelector('#'+ id + ' .exhibition_slide').style.width = totalCount * sliderItemWidth + spacing * totalCount + 'px';
    var items = document.querySelector('#'+ id + ' .exhibition_slide').children;
    for(var i=0; i<items.length; i++){
      items[i].style.width = (sliderItemWidth-spacing)+ 'px';
    }
...
var slider = new Slider('exhibition_slide', 4, 3, 1, 20);
slider.auto();